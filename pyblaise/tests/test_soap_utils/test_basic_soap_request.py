...
from pyblaise.exceptions import (
...
@pytest.mark.parametrize("host_info", host_infos)
def test__do_soap_request_raises_ServerResponse500_on_500_status_code(
    host_info, requests_mock
):
...
@pytest.mark.parametrize("host_info", host_infos)
def test__do_soap_request_raises_ServerResponse500_with_response_object(
    host_info, requests_mock
):
    requests_mock.register_uri(
        "POST", host_info["uri"], status_code=500, text="my-response-text"
    )
